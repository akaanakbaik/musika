...
async function fetchJSON(url: string, timeoutMs = 15000, retriesOrOpts: number | Record<string, any> = 1): Promise<any> {
  // Support both: fetchJSON(url, timeout, retries) and fetchJSON(url, timeout, options)
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      // Merge custom options (for POST requests)
      if (opts) {
      ...
      if (d?.status && d?.data) {
        ...
        return items.slice(0, 15).map((item: any) => {
          ...
          const vid = item.link?.match(/v=([A-Za-z0-9_-]{11})/)?.[1] ||
            item.link?.match(/youtu\.be\/([A-Za-z0-9_-]{11})/)?.[1] || "";
          return {
            videoId: vid,
            title,
            thumbnail: item.imageUrl || (vid ? `https://i.ytimg.com/vi/${vid}/hqdefault.jpg` : ""),
            duration: item.duration || "0:00",
            url: item.link || (vid ? `https://youtu.be/${vid}` : ""),
            source: "youtube",
...
async function downloadYouTube(url: string): Promise<{ downloadUrl: string; title: string; thumbnail: string; artist: string }> {
  ...
  const apis = [
    ...
    // API 2: cuki ytmp3 (user-provided, with quality=192)
    async () => {
      ...
      // Response format: { data: { metadata: { ... }, audio: { download: { downloadUrl: "..." } } } }
      const dlUrl = audio?.download?.downloadUrl || audio?.url || audio?.download_url || "";
      if (!dlUrl) throw new Error("No download URL from cuki ytmp3");
      return {
        downloadUrl: dlUrl,
        title: cleanTitle(metadata.title || audio.title || ""),
        thumbnail: metadata.thumbnail || audio.thumbnail || "",
        artist: metadata.channel || audio.channel || audio.artist || extractArtistFromTitle(metadata.title || "")
      };
    },
    // API 4: kelvdra (last resort - often returns Cloudflare 530)
    async () => {
...
async function downloadSpotify(url: string): Promise<{ downloadUrl: string; title: string; thumbnail: string; artist: string; album: string }> {
  ...
  const apis = [
    ...
    async () => {
      ...
      throw new Error("No download URL from mifinfinity");
    },
    // API 2: nexray
    async () => {
      ...
      if (d?.status && d?.data) {
        ...
        return items.slice(0, 20).map((item: any) => {
          const songId = item.link?.split("/song/")[1]?.split("?")[0] ||
            item.link?.match(/i=(\d+)/)?.[1] || String(Math.random()).slice(2);
    ...
    async () => {
      ...
      if (d?.data || d?.result || d?.results) {
        ...
        return (Array.isArray(items) ? items : []).slice(0, 20).map((item: any) => ({
          ...
          title: item.title || item.name || "",
          thumbnail: item.thumbnail || item.image || item.artwork || "",
          duration: item.duration || "0:00",
          url: item.url || item.link || "",
          source: "apple",
          artist: item.artist || item.artists || "Apple Music",
          album: item.album || ""
        }));
      }
  ...
  const apis = [
    ...
    async () => {
      ...
      if (d?.status && d?.data) {
        const items = Array.isArray(d.data) ? d.data : [];
        return items.slice(0, 15).map((item: any) => {
          const scId = item.permalink || item.permalink_url?.split("/").filter(Boolean).pop() || Math.random().toString(36).slice(2);
          const title = item.permalink?.replace(/-/g, " ") || "";
          const artist = item.permalink_url?.split("/").filter(Boolean)[1] || "SoundCloud";
          return {
            videoId: String(scId),
            title,
