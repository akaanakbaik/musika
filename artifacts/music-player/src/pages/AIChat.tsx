...
export default function AIChat({ isActive }: AIChatProps) {
  ...
  // Scroll to bottom and focus when tab becomes active
  useEffect(() => {
    if (isActive) {
      setTimeout(() => {
  ...
  const sendMessage = useCallback(async (msg?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: "assistant",
        content: lang === "en" ? "Sorry, I couldn't process that. Please try again." : "Maaf, tidak dapat memproses permintaan. Silakan coba lagi.",
        timestamp: new Date().toISOString()
      }]);
    } finally {
      setLoading(false);
  ...
  return (
    <div ref={containerRef} className={`flex flex-col h-full ${bg}`} style={{ minHeight: 0 }}>
      {/* Header */}
      <div className={`flex items-center justify-between px-4 py-3 border-b ${borderC} ${isDark ? "bg-[#121212]/90" : "bg-white/90"} backdrop-blur-sm flex-shrink-0`}>
        ...
        <button onClick={clearChat} className={`p-2 rounded-full ${isDark ? "hover:bg-white/10" : "hover:bg-black/10"} transition-colors`} title="Hapus obrolan">
            ...
            <div className={`max-w-[78%] flex flex-col gap-1 ${msg.role === "user" ? "items-end" : "items-start"}`}>
              <div
                ...
                style={{
                  ...
                  background: msg.role === "user" ? accentColor : undefined
                }}
      ...
      {/* Suggestions — show only on first visit */}
      {messages.length <= 1 && !loading && (
        <div className={`px-4 pb-2 flex-shrink-0 border-t ${borderC} pt-3`}>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {SUGGESTIONS.map(s => (
              <button
                ...
                className={`flex-shrink-0 text-xs px-3 py-2 rounded-full border ${borderC} ${isDark ? "bg-white/5 hover:bg-white/10" : "bg-black/5 hover:bg-black/10"} ${textP} transition-colors whitespace-nowrap`}
              >
                {s}
              </button>
            ))}
      ...
      {/* Input */}
      <div className={`px-4 py-3 border-t ${borderC} ${isDark ? "bg-[#121212]" : "bg-white"} flex-shrink-0`} style={{ paddingBottom: "max(env(safe-area-inset-bottom), 12px)" }}>
        <form onSubmit={e => { e.preventDefault(); sendMessage(); }} className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 disabled:opacity-40 transition-all active:scale-90"
